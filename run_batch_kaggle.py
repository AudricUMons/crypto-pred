...
def load_done_set(csv_path):
    if not os.path.exists(csv_path):
        return set()
    try:
        df = pd.read_csv(csv_path, dtype={"param_hash": "string"})
...
def main():
    parser = argparse.ArgumentParser(description="Batch runner pour simulations RF.")
    ...
    parser.add_argument("--param-grid-file", default=None,
                        help="JSON pour remplacer config.PARAM_GRID")
    ...
    parser.add_argument("--git-push-each", action="store_true",
                        help="Commit & push périodique (utile seulement hors Kaggle).")
    ...
    print(f"[{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z')}] "
          f"RUN → coin={args.coin_id}, vs={args.vs_currency}, provider={args.provider}, "
          f"results_dir={results_dir}, combos={total}")
    ...
    print(f"Resuming: {len(done)} combos already done; remaining {total - len(done)}.")
    ...
    # Itération SANS matérialiser toute la grille
    i = 0
    for params in expand_param_grid(param_grid):
        i += 1

        # Arrêt propre si on a atteint la limite
        ...
        # Data + features (avec double cache par (days, coin, vs, provider))
        ...
        if dkey not in feat_cache:
            feat_cache[dkey] = user_features.add_indicators(df_raw)
        df_feat = feat_cache[dkey]
        ...
        if final_value is None:
            print("[SKIP] résultat invalide — combo ignoré.")
            continue
        ...
        # Sauvegarde des détails si demandé
        ...
        if len(pending) >= FLUSH_EVERY:
            append_rows(live_csv, pending)
            pending.clear()

        # Checkpoint git côté GA seulement (inutile sur Kaggle)
        if args.git_push_each and (i % max(1, args.push_every) == 0):
            append_rows(live_csv, pending); pending.clear()
